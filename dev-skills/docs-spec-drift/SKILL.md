...
消えたスクリプトを名指しする `permissions`。**設定は貯まっていくので、
腐りかたも貯まる。** 詳しくは [agent-ready-repo](../agent-ready-repo/)。

**11. ロケールに頼った正規表現。**
語境界 (バックスラッシュ b) は、多バイト文字の直後ではロケールで意味が変わる。
`LANG` が未設定の環境では当たらず、**数える検査が黙って 0 を返す**。
落ちるのではなく静かに全部見逃すので、いちばん質が悪い。

```sh
grep -rnP '[^\x00-\x7f]\\b' --include='*.sh' . && bad "…"
```

この検査の説明に実例を書くと、検査が自分のコメントに当たる。**書かない。**
